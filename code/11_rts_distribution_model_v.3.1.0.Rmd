...
#          .by = UID)
artsv3 = st_read('../ARTS/ARTS_main_dataset/v.3.1.0/ARTS_main_dataset_v.3.1.0.geojson') |>
  st_make_valid() |>
  distinct() |>
  # keep one polygon per RTS only
  filter(CentroidLat == first(CentroidLat),
...
artsv1_train_fig = ggplot() +
  geom_sf(data = world_training_region,
          fill = "gray95") +
  geom_sf(data = artsv1_filter,
          aes(
            color = TrainClass,
            fill = TrainClass
          ),
...
artsv1_train_fig + artsv3_train_fig +
  new_rts_train_fig + guide_area() +
...
rts_susceptibility = rast('./sdm/gbm_kfold_10_pred_tuned_combined_v.3.1.0_filter.tif')
...
rts_probability_median = rast('./sdm/gbm_kfold_10_pred_tuned_median_v.3.1.0_filter.tif')
```
...
ggplot() +
  geom_sf(data = world, 
          ...
          inherit.aes = FALSE,
          fill = 'gray95') +
  geom_spatraster(data = rts_prob_m,
                  maxcell = 1e+07) +
  scale_fill_viridis(name = 'RTS Probability\n(%)',
                     limits = c(0, 100),
                     na.value = 'transparent') +
...
```{r}
rts_probability_median_old = rast('./sdm/gbm_kfold_10_pred_tuned_median_v.1.0.1_filter.tif')

rts_probability_diff_update = rts_probability_median - rts_probability_median_old
```
...
ggplot() +
  geom_sf(data = world, 
          ...
          inherit.aes = FALSE,
          fill = 'gray95') +
  geom_spatraster(data = rts_probability_diff_update,
                  maxcell = 1e+07) +
